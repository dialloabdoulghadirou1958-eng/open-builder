...
description: Anti-slop frontend direction for landing pages, portfolios, editorial pages, and design-forward redesigns. Use after reading the brief to infer a fitting visual system, preserve existing product constraints, and run a strict pre-flight audit.
version: 1.0.0
author: Open Builder adaptation of Leonxlnx Taste Skill
tags:
  - frontend
  - design
  - landing-page
...
> Open Builder adaptation: explicit user requirements, the project's existing stack and conventions, active tool and mode restrictions, and the tools actually available in the current run take priority over this skill. Treat absolute wording below as design guidance within those boundaries.

## Open Builder compatibility and routing

- Use this skill for landing pages, portfolios, editorial or campaign pages, and design-forward redesigns. Use `frontend-design` for routine product UI such as dashboards, settings, forms, and multi-step application flows. If both are relevant, this skill governs the marketing or portfolio surfaces and `frontend-design` governs the product surfaces.
- For an existing project, inspect its files and visual system before proposing a direction. Preserve working behavior, brand assets, framework choices, accessibility, and user-requested constraints unless the request explicitly changes them.
- Use only Open Builder tools exposed in the current run. A recommendation to use image generation, web imagery, a design system, or a dependency is conditional on availability, project fit, user authorization, and platform policy.
- Do not invent missing assets, runtime evidence, or test results. When an ideal asset cannot be produced with available tools, use the best project-compatible fallback and state the limitation without leaving incomplete placeholders unless the user asks for them.
- This Open Builder 1.0.0 adaptation retains the upstream detailed design guidance while adding routing and compatibility boundaries. Source and license details are in `SOURCE.md` and `LICENSE.txt`.
